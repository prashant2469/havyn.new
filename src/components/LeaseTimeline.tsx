...
export function LeaseTimeline({ insights, onTenantClick }: LeaseTimelineProps) {
  ...
  const groupedInsights = insights.reduce((acc, insight) => {
    const leaseEndRaw = insight.lease_to;
    if (!leaseEndRaw) return acc;
    const leaseEnd = new Date(leaseEndRaw);
    if (isNaN(leaseEnd.getTime())) return acc; // Invalid date
  ...
  const getDaysUntilLeaseEnd = (leaseEndRaw: string | null) => {
    if (!leaseEndRaw) return '—';
    const end = new Date(leaseEndRaw);
      ...
      {Object.entries(groupedInsights).map(([timeframe, tenants]) => (
        <div key={timeframe} className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
          ...
          <div className="space-y-4">
            {tenants.map(tenant => {
              const leaseEndDate = tenant.lease_to;
              const daysRemaining = getDaysUntilLeaseEnd(leaseEndDate);
              const progress = leaseEndDate && typeof daysRemaining === "number"
                ? Math.min((daysRemaining / 90) * 100, 100)
                : 0;

              return (
                <button
                  key={tenant.id || `${tenant.tenant_name}-${tenant.unit}`}
